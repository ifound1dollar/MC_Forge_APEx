...
public class ModCobaltShardLootModifier extends LootModifier {
    public static final Supplier<MapCodec<ModCobaltShardLootModifier>> CODEC = Suppliers.memoize(() ->
            RecordCodecBuilder.mapCodec(inst -> codecStart(inst).and(ForgeRegistries.ITEMS.getCodec()
                    .fieldOf("item").forGetter(m -> m.item)).apply(inst, ModCobaltShardLootModifier::new)));
    final Item item;
     ...
     */
    protected ModCobaltShardLootModifier(LootItemCondition[] conditionsIn, Item item) {
        super(conditionsIn);
        this.item = item;
    }

...
public class ModSapphireLootModifier extends LootModifier {
    public static final Supplier<MapCodec<ModSapphireLootModifier>> CODEC = Suppliers.memoize(() ->
            RecordCodecBuilder.mapCodec(inst -> codecStart(inst).and(ForgeRegistries.ITEMS.getCodec()
                    .fieldOf("item").forGetter(m -> m.item)).apply(inst, ModSapphireLootModifier::new)));
    final Item item;
     ...
     */
    protected ModSapphireLootModifier(LootItemCondition[] conditionsIn, Item item) {
        super(conditionsIn);
        this.item = item;
    }

...
public class ModCobaltSteelUpgradeTemplateModifier extends LootModifier {
    public static final Supplier<MapCodec<ModCobaltSteelUpgradeTemplateModifier>> CODEC = Suppliers.memoize(() ->
            RecordCodecBuilder.mapCodec(inst -> codecStart(inst).and(ForgeRegistries.ITEMS.getCodec()
                    .fieldOf("item").forGetter(m -> m.item)).apply(inst, ModCobaltSteelUpgradeTemplateModifier::new)));
    final Item item;
     ...
     */
    protected ModCobaltSteelUpgradeTemplateModifier(LootItemCondition[] conditionsIn, Item item) {
        super(conditionsIn);
        this.item = item;
    }

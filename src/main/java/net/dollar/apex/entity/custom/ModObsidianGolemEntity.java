...
public class ModObsidianGolemEntity extends Monster implements NeutralMob {
    ...
    public static boolean checkObsidianGolemSpawnRules(EntityType<ModObsidianGolemEntity> entityType, ServerLevelAccessor accessor,
        ...
        if (y >= 0) {
            ...
            return randomSource.nextBoolean()
                    && checkMonsterSpawnRules(entityType, accessor, spawnType, blockPos, randomSource);
        }
        ...
        if (super.doHurtTarget(targetEntity)) {
            if (targetEntity instanceof LivingEntity livingEntity) {
                ...
                if (random.nextFloat() > (this.getHealth() / this.getMaxHealth()) - 0.25f) {
                    livingEntity.setSecondsOnFire(4);       // 100% chance at 25% HP because of -0.25f above
                }
            }
    ...
    private void rangedAttackNearbyPlayers() {
        ...
        for (Player player : players) {
            ...
            if (!this.getSensing().hasLineOfSight(player)) {
                ...
                player.setSecondsOnFire(4);             // Same duration as fireball.
            }
        }
    }
     ...
     */
    @Override
    protected void dropCustomDeathLoot(@NotNull DamageSource source, int lootingLevel, boolean killedByPlayer) {
        if (!killedByPlayer) {
